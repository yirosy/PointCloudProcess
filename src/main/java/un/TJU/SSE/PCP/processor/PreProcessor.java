...
import un.TJU.SSE.PCP.util.NOIHelper;
...
public class PreProcessor extends AbstractProcessor implements Processor {
    ...
    private static int DEFAULT_OUTPUT_FILE_SIZE = 1024 * 1024 * Point.defaultPoint.getNum();
    ...
    private void fileProcess (Path inputPath, String outputPathPrefix) throws IOException {
        LOGGER.info("Start processing " + inputPath.toString() + " to " + outputPathPrefix);
        ...
        for (; position < input.length(); position+= bufferSize) {

            buffer.get(inputBuffer, 0, buffer.limit());
            tempString.append(new String(inputBuffer));
            for (int i = 0; i < buffer.limit(); i++) {
                if (NOIHelper.isNumric(tempString.charAt(i))) {
                    ...
                    if (t== maxOutputFileSize) {
                        RandomAccessFile output = new RandomAccessFile(outputPathPrefix + String.valueOf(++page), "rw");
    ...
    private void folderProcess (Path inputFolderPath, Path outputFolderPath) throws IOException {

        NOIHelper.createFolderSafely(outputFolderPath);
        ...
        if (Files.isDirectory(inputFolderPath)){
            try (DirectoryStream<Path> directoryStream = Files.newDirectoryStream(inputFolderPath)) {
                for (Path inputFilePath : directoryStream
                    ...
                    NOIHelper.createFolderSafely(outputFileRoot);
                    fileProcess(inputFilePath, outputFileRoot.toString() + "\\" + inputFilePath.getFileName().toString());
                }
            }
        } else {
            fileProcess(inputFolderPath, outputFolderPath.toString() + "\\" + inputFolderPath.getFileName().toString());
        }
    }
